...
sequenceDiagram
    ...
    alt 选择HTTP测速模式
        ...
        HTTPing->>HTTPing: 创建请求上下文RequestContext(含TLS连接器+超时+接口配置)
        note over HTTPing: RequestContext全局复用,连接按IP复用
        ...
        loop 处理每个IP(Common中循环)
             ...
             Common->>HTTPing: 调用run_test方法
             
             par 并发执行(spawn)
                 note over HTTPing: 执行HTTP测试逻辑
                 HTTPing->>Pool: 获取并发许可(acquire_permit)
                 Pool-->>HTTPing: 返回许可
                 
                 HTTPing->>HTTPing: 发送HTTP HEAD请求(复用连接)
        ...
        ICMP->>ICMP: 创建ICMP客户端(v4/v6)
        note over ICMP: 客户端被复用
        
        ICMP->>Common: 运行Ping测试
        loop 处理每个IP
             Common->>IP: 获取下一个IP
             Common->>ICMP: 调用run_test方法
             par 并发执行
                 ICMP->>Pool: 获取许可
