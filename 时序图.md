...
sequenceDiagram
    ...
    User->>Main: 启动程序并提供参数
    Main->>Args: 解析命令行参数
    Args->>Interface: 处理网络接口参数
    Interface-->>Args: 返回接口IP信息
    ...
    alt 选择HTTP测速模式
        ...
        loop 处理每个IP(并发控制)
            par 并发处理多个IP
                ...
                HTTPing->>Interface: 绑定网络接口
                Interface-->>HTTPing: 返回绑定结果
        ...
        loop 处理每个IP(并发控制)
            par 并发处理多个IP
                ...
                TCPing->>Interface: 绑定网络接口
                Interface-->>TCPing: 返回绑定结果
    ...
    alt 不禁用下载测速且有延迟测速结果
        ...
        loop 处理每个IP(串行)
            ...
            Download->>Interface: 绑定网络接口
            Interface-->>Download: 返回绑定结果
