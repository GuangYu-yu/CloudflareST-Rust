...
CloudflareST-Rust是一个命令行工具，用于评估与Cloudflare基础设施的网络性能。
...
1.  **使用默认设置进行基本HTTP ping测试**:
    
    ```text
    ./CloudflareST-Rust
    ```
    
2.  **使用自定义端口进行TCP ping测试**:
    
    ```text
    ./CloudflareST-Rust -tcp -tp 443
    ```
    
3.  **对每个IP进行10次ICMP ping测试**:
...
CloudflareST-Rust提供了众多命令行参数来自定义测试行为。
...
| 选项 | 描述 | 默认值 |
| --- | --- | --- |
| `-http` | 使用HTTP ping方法 | 默认启用 |
| `-tcp` | 使用TCP ping方法 | 禁用 |
| `-icmp` | 使用ICMP ping方法 | 禁用 |

### IP来源选项

| 选项 | 描述 | 默认值 |
| --- | --- | --- |
| `-f, --file <file>` | IP地址文件 | 内置Cloudflare IP列表 |
| `-url <address>` | 下载IP列表的URL | 无 |

### 过滤选项

| 选项 | 描述 | 默认值 |
| --- | --- | --- |
| `-sl, --speed-limit <ms>` | 最小可接受延迟 | 无限制 |
| `-tl, --time-limit <ms>` | 最大可接受延迟 | 无限制 |
| `-p, --max-loss <percent>` | 最大可接受丢包率 | 无限制 |

### 输出选项

| 选项 | 描述 | 默认值 |
| --- | --- | --- |
| `-dd, --disable-download` | 跳过下载速度测试 | 下载测试启用 |
| `-o, --output <filename>` | CSV输出文件 | 无CSV导出 |

### 测试参数

| 选项 | 描述 | 默认值 |
| --- | --- | --- |
| `-n, --num-ping <count>` | 每个IP的ping次数 | 4 |
| `-tp, --tcp-port <port>` | 测试用的TCP端口 | 443 |
| `-tn, --target-num <number>` | 显示的IP数量 | 全部IP |
...
1.  **使用特定条件测试**:
    ...
    +   使用端口443进行TCP ping测试
    +   每个IP发送10次ping
    +   过滤结果只显示延迟在30ms到200ms之间的IP
    +   仅显示前5个结果
2.  **将结果导出到CSV**:
    
3.  **使用电子表格应用程序分析CSV文件中的结果
