...
flowchart TD
    ...
    B --> J[(收集IP源<br>从文件/文本加载<br>ip.rs)]
    J --> C[初始化全局并发限制器<br>pool.rs]
    C --> D{设置全局<br>超时?}
    ...
    subgraph 延迟测速流程
        ...
        P1[初始化测试环境<br>common.rs<br>create_base_ping<br>解析IP范围/CIDR<br>创建IpBuffer/CidrState] --> P2
        P2[创建进度条<br>progress.rs<br>Bar] --> P3
        P3[创建PingMode实现<br>HttpingFactoryData/<br>TcpingFactoryData/<br>IcmpingFactoryData] --> P4
        P4[创建客户端<br>HTTP:预创建RequestContext<br>TCP:每次连接绑定接口<br>ICMP:预创建v4/v6 Client] --> P5
        P5[通过acquire_permit<br>控制并发] --> P6
    ...
    subgraph 下载测速流程
        ...
        N10 --> N15[循环读取数据块<br>GET响应体]
        N15 --> N16[累计接收数据量<br>加入采样队列]
        N16 --> N17{满500ms<br>刷新间隔?}
        N17 -->|否| N20
        N17 -->|是| N18[滑动窗口计算<br>实时速度]
        N18 --> N19[更新进度显示<br>MB/s]
        N19 --> N20{下载时长到达<br>或超时信号?}
        N20 -->|否| N15
        N20 -->|是| N21[补录最后<br>不完整窗口]
        N21 --> N22[加权计算最终速度<br>中位数-MAD]
    ...
    M & 下载测速流程 --> P[打印结果表格<br>csv.rs]
    P --> O[导出CSV结果（可选）<br>csv.rs]
    O --> Q([结束])
    ...
    class P1,P2,P3,P5,P7,P8,P9,P10,P12,P15,P17,P18,P19,P20,J,N1,N2,N3,N4,N5,N6,N7,N8,N9,N10,N11,N12,N13,N14,N15,N16,N17,N18,N19,N20,N21,N22,N23,N24,N25,N26,N29,N30 module
```